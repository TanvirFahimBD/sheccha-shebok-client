...
const MyEvents = () => {
  ...
  useEffect(() => {
    fetch(
      `https://sheccha-shebok-server.vercel.app/register/event/${user?.email}`
    )
      .then((res) => res.json())
      .then((data) => {
        setMyEvents(data);
  ...
  const handleDelete = (myEvent) => {
    ...
    if (deleteData) {
      fetch(
        `https://sheccha-shebok-server.vercel.app/register/${myEvent._id}`,
        {
          method: "DELETE",
        }
      )
        ...
        .then((data) => {
          if (data.deletedCount) {
            ...
            const userRemainingEvents = myEvents.filter(
              (userEvent) => userEvent._id !== myEvent._id
            );
