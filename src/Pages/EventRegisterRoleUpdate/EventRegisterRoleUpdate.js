...
const EventRegisterRoleUpdate = () => {
  ...
  useEffect(() => {
    fetch(`https://sheccha-shebok-server.vercel.app/register/${singleEventId}`)
      .then((res) => res.json())
      .then((data) => {
        setEvent(data);
      });
  ...
  const handleEditRole = (e) => {
    fetch("https://sheccha-shebok-server.vercel.app/users/role", {
      method: "PUT",
      headers: {
      ...
      .then((data) => {
        if (data?.modifiedCount) {
          toast.success(
            `${event.email} role ${event?.role} added Successfully`
          );
        } else {
